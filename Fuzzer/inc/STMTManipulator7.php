...
class ManipulateNodeVisitor extends NodeVisitorAbstract
{
    ...
    */
    ...
    public function beforeTraverse(array $nodes) {
        ...
        $this->before_classes = array();
        $this->before_functions = array();
    ...
    public function afterTraverse(array $nodes){
        ...
        if($this->inst_file_type != "Body"){
            ...
            foreach($this->ifConstraints as $ifConstraint){
                ...
                // php 7.x need to wrapping
                $inst_if_cond_wrapped = new Node\Stmt\Expression($inst_if_cond);
                array_push($fuzz_init_if_ast[0]->stmts, $inst_if_cond_wrapped);
            }
        ...
        // PHP 7.x need to wrapping
        $assign_entry_magic_method = new Node\Stmt\Expression($assign_entry_magic_method);
    ...
    public function enterNode(Node $node) {
        ...
        if($node instanceof Node\Stmt\Class_){
            if($node->name == NULL){
                array_push($this->before_classes, $this->currentTraverseStatus['class']);
                array_push($this->before_functions, $this->methodFunctions);
                $this->currentTraverseStatus['class'] = 'class@anonymous';
            }
            else{
                $this->currentTraverseStatus['class'] = $node->name->name;
            }
            $this->currentTraverseStatus['function'] = '';
            $this->methodFunctions = array();
        }
        if($node instanceof Node\Stmt\ClassMethod){
            $this->currentTraverseStatus['method'] = $node->name->name;
            $this->currentTraverseStatus['function'] = '';
            $this->methodFunctions = array();
        }
        if($node instanceof Node\Stmt\Function_){
            $this->currentTraverseStatus['function'] = $node->name->name;
        ...
        if($node instanceof Node\Stmt\Class_){
            foreach($node->stmts as $class_stmt){
                if($class_stmt instanceof Node\Stmt\ClassMethod and
                    ...
                    // PHP 7.x need to wrapping
                    $inst_method_call = new Node\Stmt\Expression($inst_method_call);
        ...
        if($node instanceof Node\Stmt\Echo_) {
            ...
            // PHP 7.x need to wrapping
            $inst_stmt_call = new Node\Stmt\Expression($inst_stmt_call);
            /*
            $this->addInstFuncHash(spl_object_hash($node),
                                  "FUNC",
                                  $this->currentTraverseStatus['class'],
        ...
        */
        ...
        if($node instanceof Node\Stmt\Switch_){
            ...
            foreach($node->cases as $idx => $case){
               ...
               // PHP 7.x need to wrapping
               $new_case_stmt_wrapped = new Node\Stmt\Expression($new_case_stmt);
               array_unshift($new_case_stmts, $new_case_stmt_wrapped);
        ...
        if($node instanceof Node\Stmt\If_) {
            ...
            if($node->else != NULL){
                ...
                // PHP 7.x need to wrapping
                $else_new_stmt = new Node\Stmt\Expression($else_new_stmt);
    ...
    public function leaveNode(Node $node){
        if($node instanceof Node\Stmt\Class_){
            if($node->name == NULL){
                $this->currentTraverseStatus['class'] = array_shift($this->before_classes);
                $this->methodFunctions = array_shift($this->before_functions);
            }
            $this->currentTraverseStatus['function'] = '';
        }
